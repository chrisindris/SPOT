...
with open(sys.argv[1], 'r', encoding='utf-8') as f:
        ...
        input_feat_dim = config['dataset']['input_feat_dim']
...
class SPOT(nn.Module):
    def __init__(self):
        ...
        self.feature_downsize = nn.Sequential(
                #nn.Conv1d(in_channels=input_feat_dim, out_channels=1024, kernel_size=1), # nn.Linear(input_feat_dim, feat_dim) is equivalent & slightly faster than nn.Conv2d(input_feat_dim, feat_dim, 1). NOTE: this needs to be in the state_dict, so make sure to run the pretrain before running the train.
                #nn.Conv1d(in_channels=1024, out_channels=512, kernel_size=1),
                nn.Conv1d(in_channels=input_feat_dim, out_channels=feat_dim, kernel_size=1) # let's try keeping this simple
        )
