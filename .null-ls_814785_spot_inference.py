        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            for idx, input_data, input_data_big, input_data_small,f_mask in test_loader: # Loads the features that get passed to the model, and loads the binary mask (no classes given, just the binary classes).
                ...
                for k in thres:
                    ...
                    if 1 in filt_seg_score_int: # FIX: This is only set up for a unique action in the video, since .index() on the next line will only look for the first. -done
                ...
                for locs in top_k_idx:
                    ...
                    for j in thres:
                        ...
                        #filtered_seq_int2 = ndimage.binary_fill_holes(filtered_seq_int).astype(int).tolist() # akin to the use of structuring elements, this will make regions more solid; [...0,1,1,0,1,1,0,...] -> [...0,1,1,1,1,1,0,...]# FIX: in the default setting, this fills all input holes and prevents multiple actions from being possible.
                        ...
                        if 1 in filtered_seq_int: # ensure that there exist a detection meeting the threshold
                            ...
                            for (start_pt1, end_pt1) in props_indices:
                                ...
                                # NOTE: again, we assume only one action occurs in the video. We might be able to loop over these as a fix.
                                ...
                                # NOTE: the above line cannot handle if there is more than one action in the video: in combination with the change to binary_fill_holes, this could be helped.
                                ...
                                #### get (start,end,cls_score,reg_score,label) for each top-k snip ####
                                    ...
                                    reg_score = np.mean(seq[start_pt:end_pt]) # NOTE: originally seq[start_pt+1:end_pt-1], but this may result in seq[start_pt1:end_pt-1] == [] (leading to a nan mean warning) 
                                    ...
                                    score_shift = np.amax(soft_cas_np[vid_label_id,start_pt:end_pt]) # the maximum score within the range of the action [start_pt:end_pt], assuming vid_label_id # NOTE: should probably use the predicted class for the segment, not for the whole video (something like using lbl_id rather than vid_label_id) 
